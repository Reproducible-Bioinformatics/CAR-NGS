...
#'
#running runDocker
#'      runDocker(group="docker", params=NULL, dockerID_name)
#'
#' }
#' @export
...
runDocker <- function(group="docker", params=NULL, dockerID_name, DockerSwarm=FALSE){
  ...
  if (DockerSwarm==FALSE){#Normal Docker execution
    ...
    dockerid=readLines(dockerID_name, warn = FALSE)
    ...
    if(as.numeric(dockerExit)!=0){
        ...
        cat(paste("Please send to raffaele.calogero@unito.it this error: Docker failed exit 0,\n
                  ...
                  which is envirved in your working folder:\n", substr(dockerid,1,12),".log\n", sep=""))
  ...
  {#Swarm Docker execution
    ...
    params= strsplit(params , split = dockerID_name)[[1]][2]#remove dockerID file
